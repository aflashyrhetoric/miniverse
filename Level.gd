...
func _ready():
	...
	if out_of_bounds != null:
		out_of_bounds.connect("body_entered", Callable(self, "handle_out_of_bounds"))
	...
	for boundary in boundaries.get_children():
		...
		boundary.connect("area_entered", Callable(self, "change_room_to").bind(boundary.get_child(0).name))
...
func create_land_dust(_feet_position):
	...
	add_child(land_dust)
	await get_tree().create_timer(WorldVars.DUST_LIFETIME).timeout
