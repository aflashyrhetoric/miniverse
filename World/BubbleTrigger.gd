...
func _ready() -> void:
	...
	Events.connect("ellie_entered_action_range", Callable(self, "enter_interaction_range"))
	Events.connect("ellie_exited_action_range", Callable(self, "exit_interaction_range"))

	# detect when a shard hits the orb area
	shard_hit_radius.connect("body_entered", Callable(self, "handle_shard_hit"))
...
func handle_shard_hit(_shard):
	if _ellie_is_within_range:
		...
		_shard.get_node("Sprite2D").visible = false
